...
class Module(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        print(f"Epoch: {self.current_epoch}, Loss: {loss}, MAE: {mae}, PSNR: {psnr}")
...
if __name__ == '__main__':
    ...
    # images_ref = [image for image in images]
    ...
    if os.path.exists('new_lowres'):
        os.system('rm -rf new_lowres')

    if os.path.exists('new_highres'):
        os.system('rm -rf new_highres')
    
    if os.path.exists('new_highres_pred'):
        os.system('rm -rf new_highres_pred')    

    if not os.path.exists('new_lowres'):
        os.makedirs('new_lowres')

    if not os.path.exists('new_highres'):
        os.makedirs('new_highres')

    if not os.path.exists('new_highres_pred'):
        os.makedirs('new_highres_pred')
    ...
    for i in range(len(images_l)):
        ...
        curr_img_l=((curr_img_l)*255.0).astype(np.uint8)
        curr_img_l=curr_img_l[...,::-1]
        cv2.imwrite('new_lowres/'+file_names[i], curr_img_l)

        curr_img_h=torch.permute(curr_img_h.detach().cpu(), (0, 2, 3, 1)).numpy()[0]
        curr_img_h=((curr_img_h)*255.0).astype(np.uint8)
        curr_img_h=curr_img_h[...,::-1]
        cv2.imwrite('new_highres_pred/'+file_names[i], curr_img_h)

        ref_img=torch.permute(ref_img.detach().cpu(), (0, 2, 3, 1)).numpy()[0]
        ref_img=((ref_img)*255.0).astype(np.uint8)
        ref_img=ref_img[...,::-1]
        cv2.imwrite('new_highres/'+file_names[i], ref_img)
